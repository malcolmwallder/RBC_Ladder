...
function AdminPanel({ ladder, settings, oomLedger, onAdd, onDelete, onEdit, onImport, onSaveSettings, onMove, onAwardBonus, onDeleteOomEntry, onClose, onLock }) {
  ...
  const [editForm, setEditForm] = useState({ name: "", surname: "", cell: "", grade: "", hcap: "", years: "" });
                    ...
                    <>
                      ...
                      <td style={{ padding: "6px 6px" }}>
                        <input
                          ...
                          style={{ ...tableInputStyle, width: 52 }}
                        />
                      </td>
                    </>
                  ) : (
...
export default function App() {
  ...
  const issueChallenge = async (opponentId, details = {}) => {
    ...
    // Order of Merit: challenger earns points for issuing, and their
    // "last active" clock resets (this is what the inactivity penalty checks).
    ...
    const nextLedger = [
      ...(oomLedger || []),
      {
        ...
        type: "challenge",
        note: `Challenged ${displayName(opp)}`,
        at: nowTs,
      },
    ];
    const nextTracking = {
