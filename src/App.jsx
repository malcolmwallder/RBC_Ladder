...
// Results can be recorded (and undone) by the two bowlers in a challenge.
// Set this to true to also let an unlocked admin do it (handy if a player
// can't get to their phone). Set to false for strictly "players only".
const ADMIN_CAN_RECORD_RESULTS = true;
...
export default function App() {
  ...
  // Only the two bowlers in a challenge (or, if enabled, an unlocked admin)
  // may record its result or bring a recorded result back.
  const canRecordResult = (challenge) =>
    (!!myId && (myId === challenge.challengerId || myId === challenge.opponentId)) ||
    (ADMIN_CAN_RECORD_RESULTS && isAdmin);

  const recordResult = async (challenge, winnerId) => {
    if (busy) return;
    if (!canRecordResult(challenge)) {
      showToast("Only the two bowlers in this challenge can record the result.", "error");
      return;
    }
    if (winnerId !== challenge.challengerId && winnerId !== challenge.opponentId) return;
    ...
    // Everything we change is remembered on the challenge so the result can be
    // cleanly reversed later ("bring back from history").
    const undoInfo = { moved: false, shiftedIds: [], prevMovements: {}, ledgerIds: [] };
    ...
    if (winnerId === challenge.challengerId && cIdx > oIdx && cIdx >= 0 && oIdx >= 0) {
      // Players from the opponent down to just above the challenger each slide
      // down one place. Record who they are, and their previous arrows.
      const shifted = ladderCopy.slice(oIdx, cIdx);
      undoInfo.moved = true;
      undoInfo.shiftedIds = shifted.map((b) => b.id);
      undoInfo.prevMovements[challenge.challengerId] = ladderCopy[cIdx].movement ?? null;
      shifted.forEach((b) => { undoInfo.prevMovements[b.id] = b.movement ?? null; });
    ...
    const mkId = (suffix) => `oom-${nowTs}-${Math.random().toString(36).slice(2, 7)}${suffix}`;
    const newEntries = [
      {
        id: mkId("c"),
    ...
    undoInfo.ledgerIds = newEntries.map((e) => e.id);

    const newChallenges = (challenges || []).map((c) =>
      c.id === challenge.id
        ? { ...c, status: "completed", winnerId, completedAt: nowTs, undoInfo }
        : c
    );

    await persistLadder(newLadder);
    await persistChallenges(newChallenges);
    await persistOomLedger([...(oomLedger || []), ...newEntries]);
  ...
  // Reverses a recorded result: challenge goes back on the rail, the ladder
  // order/arrows are restored, and the Order of Merit points it earned are removed.
  const undoResult = async (challenge) => {
    if (busy) return;
    setConfirmUndoId(null);
    if (!canRecordResult(challenge)) {
      showToast("Only the two bowlers in this challenge can bring it back.", "error");
      return;
    }
    const cId = challenge.challengerId;
    const oId = challenge.opponentId;

    // The ladder allows one live challenge per bowler, so both must be free.
    if (idInAnyPending(cId) || idInAnyPending(oId)) {
      showToast(
        "One of these bowlers already has a challenge in progress. Finish or cancel that one first.",
        "error"
      );
      return;
    }

    const info = challenge.undoInfo || null; // absent on results recorded before this feature
    const cCur = ladder.findIndex((b) => b.id === cId);

    // ---- Work out how to put the ladder back ----
    let newLadder = null; // null = leave the ladder untouched
    let ladderNote = "";
    const winnerIsChallenger = challenge.winnerId === cId;
    const legacySpan = Math.max(0, (challenge.challengerPosition || 0) - (challenge.opponentPosition || 0));
    const expectMoved = info ? info.moved : winnerIsChallenger && legacySpan > 0;

    if (expectMoved) {
      const span = info ? info.shiftedIds.length : legacySpan;
      const below = cCur >= 0 ? ladder.slice(cCur + 1, cCur + 1 + span).map((b) => b.id) : [];
      // The result is only safely reversible if the challenger still sits directly
      // above the same group of players they jumped over.
      const stillIntact =
        span > 0 &&
        below.length === span &&
        below[0] === oId &&
        (!info || info.shiftedIds.every((id, i) => id === below[i]));

      if (stillIntact) {
        const copy = ladder.map((b) => ({ ...b }));
        const [mover] = copy.splice(cCur, 1);
        copy.splice(cCur + span, 0, mover);
        const touched = new Set([cId, ...below]);
        copy.forEach((b, i) => {
          if (!touched.has(b.id)) return;
          const prev = info && info.prevMovements ? info.prevMovements[b.id] : null;
          copy[i] = { ...b, movement: prev ?? null };
        });
        newLadder = copy;
      } else if (isAdmin) {
        ladderNote = " The ladder order has changed since, so it was left as-is \u2014 adjust it in the admin panel if needed.";
      } else {
        showToast(
          "The ladder has moved on since this result, so it can't be undone automatically. Ask an admin to bring it back.",
          "error"
        );
        return;
      }
    ...
    // ---- Work out which Order of Merit entries to remove ----
    const removeIds = new Set(info ? info.ledgerIds : []);
    if (!info) {
      const loserId = winnerIsChallenger ? oId : cId;
      const wanted = [
        [cId, "challenge"],
        [challenge.winnerId, "win"],
        [loserId, "loss"],
      ];
      for (const [bowlerId, type] of wanted) {
        const hit = (oomLedger || []).find(
          (e) =>
            e.bowlerId === bowlerId &&
            e.type === type &&
            !removeIds.has(e.id) &&
            e.at >= (challenge.completedAt || 0) - 1000 &&
            e.at <= (challenge.completedAt || 0) + 60000
        );
        if (hit) removeIds.add(hit.id);
      }
    }
    const newLedger = (oomLedger || []).filter((e) => !removeIds.has(e.id));

    // ---- Put the challenge back on the rail ----
    const newChallenges = (challenges || []).map((c) => {
      if (c.id !== challenge.id) return c;
      // eslint-disable-next-line no-unused-vars
      const { winnerId, completedAt, undoInfo, ...rest } = c;
      return { ...rest, status: "pending", reopenedAt: Date.now() };
    });

    setBusy(true);
    if (newLadder) await persistLadder(newLadder);
    await persistChallenges(newChallenges);
    await persistOomLedger(newLedger);
    setBusy(false);

    const cb = ladder.find((b) => b.id === cId);
    const ob = ladder.find((b) => b.id === oId);
    showToast(
      `Result undone \u2014 ${displayName(cb)} vs ${displayName(ob)} is back on the rail.${ladderNote}`
    );
  };
              ...
              {pendingChallenges.map((c) => (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    >
                      {myId
                        ? `Only ${c.challenger.name} or ${c.opponent.name} can record this result.`
                        : `Pick who you are to record a result \u2014 only ${c.challenger.name} or ${c.opponent.name} can.`}
                    </div>
                  )}
                  {canEditSchedule(c) && (
                      ...
                      >
                        ...
                        {canRecordResult(c) && (
                          <div style={{ marginTop: 8 }}>
                            {confirmUndoId === c.id ? (
                              <div>
                                <div style={{ fontSize: 12, color: COLORS.ink, marginBottom: 6, lineHeight: 1.4 }}>
                                  This puts the challenge back on the rail, restores the ladder order and
                                  removes the points it earned.
                                </div>
                                <div style={{ display: "flex", gap: 8 }}>
                                  <button
                                    disabled={busy}
                                    onClick={() => undoResult(c)}
                                    style={{
                                      flex: 1,
                                      padding: "6px 0",
                                      borderRadius: 8,
                              ...
                              >
                                Recorded by mistake? Bring it back
                              </button>
                            )}
                          </div>
                        )}
